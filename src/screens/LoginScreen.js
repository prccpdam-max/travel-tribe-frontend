  ...
  return (
    <View style={s.container}>
      ...
      <TouchableOpacity style={s.btn} onPress={handleAuth} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>{isRegister ? "???????????" : "Login"}</Text>}
      </TouchableOpacity>
      <TouchableOpacity onPress={() => setIsRegister(!isRegister)}>
        <Text style={s.switchText}>{isRegister ? "???????????? Login" : "?????????????? ???????????"}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  title: { color: "#7B9FFF", fontSize: 36, fontWeight: "800", textAlign: "center", marginBottom: 8 },
